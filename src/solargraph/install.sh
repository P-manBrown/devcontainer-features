...
cat <<-'EOF' > /usr/local/share/solargraph-init.sh
	...
	if [[ -n "${SOLARGRAPH_CACHE}" ]]; then
	  cache_paths="$(
	    find "${USER_HOME}/.solargraph/cache" -mindepth 1 -maxdepth 1
	  )"
	...
	if [[ "${LOCAL_CONFIG_DIR}" == 'global' ]]; then
	  ...
	  mkdir_and_mv "/tmp/solargraph/.solargraph.yml" "${global_config}"
	else
	  mkdir_and_mv "/tmp/solargraph/.solargraph.yml" "${LOCAL_CONFIG_DIR}"
	  cd "${LOCAL_CONFIG_DIR}"
